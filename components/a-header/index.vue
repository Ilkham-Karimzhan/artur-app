<script setup lang="ts">
  ...
  const links: string[] = ['Shop', 'Men', 'Women', 'Combos', 'Joggers'];

  const icons: string[] = [HeartIcon, UserIcon, CartIcon];

  const input = ref<string>('');
</script>
<template>
  <nav
    class="flex items-center justify-between max-w-[1240px] mx-auto py-[31px]"
  >
    ...
    <ul class="flex gap-10">
      <li
        class="text-[#807D7E] cursor-pointer transition hover:text-[#3C4242] text-[22px] font-bold"
    ...
    <ul class="flex gap-3">
      <li
        v-for="(svg, idx) in icons"
        :key="idx"
        class="icon cursor-pointer p-3 bg-[#F6F6F6] transition hover:bg-[#01add5] hover:text-white text-[20px] rounded-lg"
      >
        <component :is="svg" filled />
      </li>
    </ul>
  </nav>
</template>
<style scoped lang="scss">
  .icon:hover > * {
    filter: brightness(0) saturate(100%) invert(100%) sepia(0%) saturate(0%)
      hue-rotate(198deg) brightness(105%) contrast(102%);
  }
</style>
